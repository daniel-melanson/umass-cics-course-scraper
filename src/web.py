...
def get_course_frequency():
    ...
    def cics_course_frequency(elem: Tag):
        return (
            text_of(elem.select_one("td:first-child")) + " " + text_of(elem.select_one("td:nth-child(2)")),
...
def scrape_courses():
    ...
    for year in range(current_year, 17, -1):
        for id in [7, 3]:
            soup = scrape(f"https://web.cs.umass.edu/csinfo/autogen/cicsdesc1%s%s.html" % (year, id))
            ...
            for header in soup.select("h2:not(:first-child)"):
                ...
                title_match = re.match(r"^(CICS|COMPSCI|INFO|INFOSEC)\s*(\w+):\s*([\w -:]+)", raw_title, re.IGNORECASE)
                ...
                if next_sibling.name == "h3":
                    if instructor_match := re.match(r"^(Instructor\(s\): )(.+)", text_of(next_sibling), re.IGNORECASE):
                ...
                if course_id in course_map:
                    course_instructors = course_map[course_id]["instructors"]

                    for name in session_staff:
                        course_instructors.add(name)
                else:
    ...
    for i in range(start, min, -1):
        soup = scrape(f"https://www.math.umass.edu/course-descriptions?semester_tid=%d" % i)

        for article in soup.select("div > article"):
            raw_title = text_of(article.select_one("div[class='field-title clearfix'] > h3"))
            title_match = re.match(r"^(MATH|STAT|HONORS)\s*(\w+)(\.\d*)?:\s*([\w -:]+)", raw_title, re.IGNORECASE)
            ...
            course_description = text_of(article.select_one("div[class='field-course-descr-description inline clearfix'] > div > p"))
...
def retrieve_staff_information():
    ...
    for div_element in soup.select("div.view-content > div"):
        def div_get(class_name, selector='div > div > a'):
            return div_element.select_one(f"div[class='%s'] > %s" % (class_name, selector))

        name_link = div_get("field field-name-title field-type-ds field-label-hidden")
        raw_name = text_of(name_link)
        ...
        staff_list.append({
            ...
            "title": text_of(div_get("field field-name-field-position field-type-text field-label-hidden", "div > div")),
            "photo": div_element.select_one("img")["src"],
            "email": text_of(div_get("field field-name-field-email field-type-email field-label-inline clearfix")),
            "website": "https://www.cics.umass.edu" + website if website[0] == "/" else website, 
            "courses": []
        })
...
def get_academic_schedule():
    soup = scrape("https://www.umass.edu/registrar/calendars/academic-calendar")

    semester_list = []
    for header in soup.select(".field-item h3"):
        semester_title = text_of(header)

        match = re.match(r"^(university )?(spring|summer|fall) (\d{4})", semester_title, re.IGNORECASE)
        if not match:
            continue
        ...
        for event_element in table.select("tr"):
            ...
            native_time = datetime.strptime(f"%s %s %s 00:00:00" % (day_number, month_name, adjusted_year), "%d %B %Y %H:%M:%S")
